...
# === Título ===
st.write(''' # Predicción de Costo de Actividades Diarias ''')
st.markdown("Ingresa los datos de tu próxima actividad para estimar cuánto gastarás.")
# st.image("logo.png", use_column_width=True) # Commented out as logo.png was not found

# === Cargar y preparar los datos ===
# Aquí debes tener tu archivo 'registrodatos.csv' en la misma carpeta que este script
try:
    df = pd.read_csv('registrodatos.csv', encoding='latin1')
except FileNotFoundError:
    st.error("⚠️ No se encontró el archivo 'registrodatos.csv'. Asegúrate de incluirlo.")
    st.stop()

# Eliminar columnas innecesarias
df.drop(['Número', 'Fecha (dd/mm/aa)', 'Nombre actividad'], axis=1, inplace=True, errors='ignore')
...
# Mapeo según la descripción oficial
...
# Eliminar filas con mapeo inválido
...
momento_label = st.selectbox('Momento del día:', ['Mañana', 'Tarde', 'Noche'])
momento_cod = 1 if momento_label == 'Mañana' else (2 if momento_label == 'Tarde' else 3)
...
if st.button("🔍 Predecir costo"):
    ...
    if prediccion > presupuesto:
        st.warning("¡Cuidado! El gasto estimado supera tu presupuesto.")
    else:
        st.success("¡Excelente! Estás dentro de tu presupuesto.")